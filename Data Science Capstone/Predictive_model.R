...
setkeyv(ngrams_dict, c("last_wrt", "word_n1", "word_n2", "num_gram"))

{to_pred <- "you shall not" %>%
   str_to_lower() %>%
   str_trim() %>%
   str_remove_all("[_$&+,:;=?@#~{}|`\\<>.^*°()%!\\-\\[\\]/0-9]")
...
one_match <- trn_sumdfm[.(2, word(to_pred,-1)),
                            .(feature, frequency, pred_word)]


#----
# Search for matching trigrams
two_match <- trn_sumdfm[.(3, word(to_pred,-1), word(to_pred,-2)),
                        .(feature, frequency, pred_word)]


#----
# Search for matching fourgrams
three_match <- trn_sumdfm[.(4, word(to_pred,-1), word(to_pred,-2), word(to_pred,-3)),
                          .(feature, frequency, pred_word)]


#----
# Gather result
rbind(one_match, two_match, three_match)}
